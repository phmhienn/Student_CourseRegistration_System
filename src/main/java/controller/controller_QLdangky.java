...
public class controller_QLdangky {
    ...
    private void loadLichHocTheoPhong() {
        ...
        String sql = """
            SELECT DISTINCT lhp1.thu, lhp1.ca_hoc
                  FROM LopHocPhan lhp1
                  WHERE lhp1.ma_mon = ?
                    AND lhp1.ma_hoc_ky = ?
                    AND lhp1.phong_hoc = ?
                    AND NOT EXISTS (
                        ...
                        JOIN LopHocPhan lhp2 ON dk.ma_lhp = lhp2.ma_lhp
                        WHERE lhp2.phong_hoc = ?
                          AND lhp2.ma_hoc_ky = ?
                          AND lhp2.thu = lhp1.thu
                          AND lhp2.ca_hoc = lhp1.ca_hoc
                    )
        """;
