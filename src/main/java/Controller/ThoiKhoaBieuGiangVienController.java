/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package Controller;

/**
 *
 * @author HUY
 */
import dao.DanhMucDAO;
import Model.ComboItem;
import Model.ThoiKhoaBieuGiangVien;
import Service.ThoiKhoaBieuGiangVienService;
import View.FrmThoiKhoaBieuGiangVien;

import javax.swing.table.DefaultTableModel;
import java.util.List;
public class ThoiKhoaBieuGiangVienController {
    private final FrmThoiKhoaBieuGiangVien view;
    private final String maGv;
    private final ThoiKhoaBieuGiangVienService service = new ThoiKhoaBieuGiangVienService();
    private final DanhMucDAO dmDao = new DanhMucDAO();

    public ThoiKhoaBieuGiangVienController(FrmThoiKhoaBieuGiangVien view, String maGv) {
        this.view = view;
        this.maGv = maGv;
        init();
    }

    private void init() {
        loadHocKy();
        bindEvents();
    }

    private void loadHocKy() {
        view.cboHocKy.removeAllItems();
        for (ComboItem hk : dmDao.getHocKy()) {
            view.cboHocKy.addItem(hk);
        }
    }

    private void bindEvents() {
        view.cboHocKy.addActionListener(e -> loadTKB());
    }

    private void loadTKB() {
        ComboItem hk = (ComboItem) view.cboHocKy.getSelectedItem();
        if (hk == null) return;

        List<ThoiKhoaBieuGiangVien> list = service.getTKB(maGv, hk.getId());

        DefaultTableModel model = (DefaultTableModel) view.tblTKB.getModel();
        model.setRowCount(0);

        for (ThoiKhoaBieuGiangVien t : list) {
            model.addRow(new Object[]{
                t.getTenMon(),
                t.getMaLhp(),
                "Thứ " + t.getThu(),
                t.getTietBatDau(),
                t.getSoTiet(),
                t.getPhongHoc()
            });
        }
    }
}
